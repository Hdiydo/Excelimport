...
import org.zxiat.entity.target.Claim;
import org.zxiat.entity.target.Entity;
import org.zxiat.entity.target.Gene;
import org.zxiat.entity.target.Snak;
...
public class CommonTransformer {
    ...
    public Future<Map<String, String>> getFromObject(List<Map<String, String>> objects, HashMap<String, String> labelsMap, HashMap<String, String> descsMap,
        ...
        for (Object object : objects) {
            ...
            for (String colName : dbObject.keySet()) {
                if (dbObject.get(colName) != null && !"".equals(dbObject.get(colName))) {
                    if (labelcols.contains(colName)) {
                        ...
                        Gene gene = new Gene(source, type, "#fff", new Date());
                        c.setSrc(gene);
            ...
            entity.setAliases(aliases); //setTags   //

            entity.setMod(new Date());
            entity.setGenes(Arrays.asList(new Gene(source, type, "#fff", new Date())));
    ...
    private void saveEntity(Entity entity, String collectionName) {
        String uuid = getOrderIdByUUId();
        entity.setId(uuid);
